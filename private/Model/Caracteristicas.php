...
class Caracteristicas extends Connection
{
    ...
    public function updateStatus(int $id){
        $this->id = $id;
        $query = "UPDATE `gc_equipos`.`gce_caracteristicas` SET gce_estado = 0 WHERE id = ?";
         // Prepara la sentencia
        $sql = $this->DB->prepare($query);
         // Ejecuta la consulta
         try {
            ...
            return ["status" => 200, "response" => false, "message" => 'No es posible actualizar el registro: ' . $th->getMessage()];
        }
    }
    ...
    // Código de consulta
        $query = "DELETE FROM `gc_equipos`.`gce_caracteristicas` WHERE gce_id = ?";
